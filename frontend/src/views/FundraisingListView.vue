...
const loadFundraisingList = async () => {
  try {
    ...
    } else if (currentStatus.value === 'ended') {
      const [completedRes, terminatedRes] = await Promise.all([
        fundraisingApi.getFundraisingByStatus('completed'),
        fundraisingApi.getFundraisingByStatus('terminated')
      ])
      fundraisingList.value = [...(completedRes.data || []), ...(terminatedRes.data || [])]
      return
    } else {
